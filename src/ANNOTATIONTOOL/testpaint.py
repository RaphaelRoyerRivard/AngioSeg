...
class ScribbleArea(QtGui.QWidget):
    ...
    def __init__(self, image, canvas):
        ...
        self.setPenColor = QtGui.QColor(255,0,0,255)
...
#       print "event.pos() = %s" % event.pos()
        if event.button() == QtCore.Qt.LeftButton:
            ...
            self.myPenColor = self.setPenColor

        if event.button() == QtCore.Qt.RightButton:
            self.lastPoint = event.pos()
            self.scribbling = True
            self.myPenColor = QtGui.QColor(0,0,0,0)
    ...
    def paintEvent(self, event):
        ...
        #painter.scale(self.canvas.scaleFactor, self.canvas.scaleFactor)
        rect = event.rect()
        #painter.drawImage(QtCore.QRect(0,0,rect.size().width(), rect.size().width()), self.image)
        painter.drawImage(QtCore.QRect(0, 0, self.image.width(), self.image.width()), self.image)
        #self.resize(int(self.image.width()*self.canvas.scaleFactor), int(self.image.height()*self.canvas.scaleFactor))
...
#         self.scaleFactor = min(self.canvas.width()/float(self.image.width()), self.canvas.height()/float(self.image.height()))
#         newWidth = self.image.width()*self.scaleFactor
#         newHeight = self.image.height()*self.scaleFactor
