...
def get_image_summary(img):
    disp = tf.slice(img, (0, 0, 0, 0), (1, -1, -1, 3))
    disp = tf.reshape(disp, [1, tf.shape(disp)[1], tf.shape(disp)[2], tf.shape(disp)[3]])
    return disp
...
def model(data, train, imagesize, numchannels, numlabels):
  ...
  if os.name == 'nt':
      ...
      conv5b_biases = tf.Variable(tf.constant(0.0, shape=[512]))

      conv5c_weights = tf.get_variable("conv5c_weights", shape=[3, 3, 512, 512], initializer=tf.contrib.layers.xavier_initializer())
      conv5c_biases = tf.Variable(tf.constant(0.0, shape=[512]))

      deconv5_weights = tf.get_variable("deconv5_weights", shape=[2, 2, 256, 512], initializer=tf.contrib.layers.xavier_initializer())
      deconv5_biases = tf.Variable(tf.constant(0.0, shape=[256]))

      conv5de_weights = tf.get_variable("conv5de_weights", shape=[3, 3, 256, 256], initializer=tf.contrib.layers.xavier_initializer())
      conv5de_biases = tf.Variable(tf.constant(0.0, shape=[256]))

      deconv4_weights = tf.get_variable("deconv4_weights", shape=[2, 2, 128, 256], initializer=tf.contrib.layers.xavier_initializer())
      deconv4_biases = tf.Variable(tf.constant(0.0, shape=[128]))

      conv4de_weights = tf.get_variable("conv4de_weights", shape=[3, 3, 128, 128], initializer=tf.contrib.layers.xavier_initializer())
      conv4de_biases = tf.Variable(tf.constant(0.0, shape=[128]))

      deconv3_weights = tf.get_variable("deconv3_weights", shape=[2, 2, 64, 128], initializer=tf.contrib.layers.xavier_initializer())
      deconv3_biases = tf.Variable(tf.constant(0.0, shape=[64]))

      conv3de_weights = tf.get_variable("conv3de_weights", shape=[3, 3, 64, 64],initializer=tf.contrib.layers.xavier_initializer())
      conv3de_biases = tf.Variable(tf.constant(0.0, shape=[64]))

      deconv2_weights = tf.get_variable("deconv2_weights", shape=[2, 2, 32, 64], initializer=tf.contrib.layers.xavier_initializer())
      deconv2_biases = tf.Variable(tf.constant(0.0, shape=[32]))

      conv2de_weights = tf.get_variable("conv2de_weights", shape=[3, 3, 32, 32],initializer=tf.contrib.layers.xavier_initializer())
      conv2de_biases = tf.Variable(tf.constant(0.0, shape=[32]))

      deconv1_weights = tf.get_variable("deconv1_weights", shape=[2, 2, 16, 32], initializer=tf.contrib.layers.xavier_initializer())
      deconv1_biases = tf.Variable(tf.constant(0.0, shape=[16]))

      conv1de_weights = tf.get_variable("conv1de_weights", shape=[3, 3, 16, 16],initializer=tf.contrib.layers.xavier_initializer())
      conv1de_biases = tf.Variable(tf.constant(0.0, shape=[16]))
      ...
      convlast_biases = tf.Variable(tf.constant(0.0, shape=[numlabels]))
  ...
  out1nl = tf.nn.relu(tf.nn.bias_add(out1, conv1_biases))
  ...
  return None, None, outputmap, conv1_weights, conv1_biases, outputshape, out1,  out2
...
def build_feedfoward_ops(train_data_node, train_labels_node, train, imagesize, numchannel, numlabels):

  hidden, logits,outputmap,  conv1_weights, conv1_biases, out1, out1nl, out2 =  model(train_data_node, train, imagesize, numchannel, numlabels)
  ...
  if train:
    ...
    tf.summary.image('label', get_image_summary(train_labels_node))
    ...
    tf.summary.image('outputmap', get_image_summary_disp(outputmap))
    tf.summary.image('input', get_image_summary(train_data_node))
    tf.summary.image('outmapmax', get_image_summary_disp_grayscale(outmapmax))
    tf.summary.scalar('Accuracy', accuracyt)

    return logits, None, outputmap, loss, optimizer, accuracyt, out1
  else:
    accuracyeval, outmapmax = error_rate(outputmap, train_labels_node, numlabels)
    tf.summary.image('outputmap_test', get_image_summary_disp(outputmap))
    tf.summary.image('input_test', get_image_summary(train_data_node))
    tf.summary.image('outmapmax_test', get_image_summary_disp_grayscale(outmapmax))
    return hidden, None, outputmap, accuracyeval,  conv1_weights, conv1_biases, out1, out1nl, out2
