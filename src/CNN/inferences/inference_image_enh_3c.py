...
def read_and_decode_angio_v0(filename, numepoch=None):
  ...
  image = tf.image.decode_jpeg(str1, channels=3)
...
def inferfromimage(image, imageenh, checkpoint, tfrecordsname, percresz):
  ...
  with tf.Session(config=config) as sess:
    ...
    likelihoodvessels=(np.clip(np.divide(outputmapvrz[:, :, 1], outputmapvrz[:, :, 0]) * 100, 0, 255).astype(np.uint8))

    probavessels = likelihoodvessels

    # if we want the raw proba
    #probavessels2 = (np.clip(outputmapvrz/np.max(outputmapvrz)*255, 0, 255).astype(np.uint8))[0:height, 0:width]
    #probavessels2 = np.concatenate((probavessels2, np.zeros((probavessels2.shape[0], probavessels2.shape[1], 1))), axis=2)

    #probavessels = np.clip(np.add(probavessels2[:,:,1], -probavessels2[:,:,0]*0.2)+30, 0, 255).astype(np.uint8)[0:height, 0:width]
    ...
    indicenew = misc.imresize(indicenew, 1 / percresz, interp='nearest')
      probavessels = misc.imresize(probavessels, 1 / percresz, interp='nearest')
    return indicenew[0:height, 0:width], probavessels[0:height, 0:width], \
           likelihoodvessels


def main(argv):  # pylint: disable=unused-argument
  opts, args = getopt.getopt(argv[1:], "hi:o:e:c:r:")
  print(opts)
  resol=1.0
  inputfileenh=None
  for opt, arg in opts:
  ...
  inferfromimagemain(inputfile, inputfileenh, checkpoint, outputfile, resol)
