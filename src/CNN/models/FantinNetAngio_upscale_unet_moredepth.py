...
def model(data, train, imagesize, numchannels, numlabels):
  ...
  if os.name == 'nt':
      ...
      conv6_weights = tf.get_variable("conv6_weights", shape=[3, 3, 512, 1024], initializer=tf.contrib.layers.xavier_initializer())
      conv6_biases = tf.get_variable("conv6_biases", initializer = tf.constant(0.0, shape=[1024]))

      conv6b_weights = tf.get_variable("conv6b_weights", shape=[3, 3, 1024, 1024], initializer=tf.contrib.layers.xavier_initializer())
      conv6b_biases = tf.get_variable("conv6b_biases",  initializer = tf.constant(0.0, shape=[1024]))

      conv6c_weights = tf.get_variable("conv6c_weights", shape=[3, 3, 1024, 1024], initializer=tf.contrib.layers.xavier_initializer())
      conv6c_biases = tf.get_variable("conv6c_biases", initializer = tf.constant(0.0, shape=[1024]))

      conv6de_weights = tf.get_variable("conv6de_weights", shape=[3, 3, 1024, 512], initializer=tf.contrib.layers.xavier_initializer())
      conv6de_biases = tf.get_variable("conv6de_biases",  initializer = tf.constant(0.0, shape=[512]))
      ...
      conv5de_weights = tf.get_variable("conv5de_weights", shape=[3, 3, 1024, 256], initializer=tf.contrib.layers.xavier_initializer())
      ...
      conv4de_weights = tf.get_variable("conv4de_weights", shape=[3, 3, 512, 128], initializer=tf.contrib.layers.xavier_initializer())
      ...
      conv3de_weights = tf.get_variable("conv3de_weights", shape=[3, 3, 256, 64],initializer=tf.contrib.layers.xavier_initializer())
      ...
      conv2de_weights = tf.get_variable("conv2de_weights", shape=[3, 3, 128, 32],initializer=tf.contrib.layers.xavier_initializer())
      ...
      conv1de_weights = tf.get_variable("conv1de_weights", shape=[3, 3, 64, 16],initializer=tf.contrib.layers.xavier_initializer())
  ...
  out2cr = tf.nn.max_pool(out1nlc,
                        ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1],
                        padding='SAME')
  out2 = tf.nn.conv2d(out2cr,
                      conv2_weights,
                      strides=[1, 1, 1, 1],
                      padding='SAME')
  ...
  out3cr = tf.nn.max_pool(relu,
                        ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1],
                        padding='SAME')
  out3 = tf.nn.conv2d(out3cr,
                      conv3_weights,
                      strides=[1, 1, 1, 1],
                      padding='SAME')
  ...
  out4cr = tf.nn.max_pool(relu,
                        ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1],
                        padding='SAME')

  conv = tf.nn.conv2d(out4cr,
                      conv4_weights,
                      strides=[1, 1, 1, 1],
                      padding='SAME')
  ...
  out5cr = tf.nn.max_pool(relu,
                        ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1],
                        padding='SAME')

  conv = tf.nn.conv2d(out5cr,
                      conv5_weights,
                      strides=[1, 1, 1, 1],
                      padding='SAME')
  ...
  out6cr = tf.nn.max_pool(relu,
                        ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1],
                        padding='SAME')

  conv = tf.nn.conv2d(out6cr,
                      conv6_weights,
                      strides=[1, 1, 1, 1],
                      padding='SAME')

  relu = tf.nn.relu(tf.nn.bias_add(conv, conv6_biases))
  out6b = tf.nn.conv2d(relu,
                      conv6b_weights,
                      strides=[1, 1, 1, 1],
                      padding='SAME')
  relu = tf.nn.relu(tf.nn.bias_add(out6b, conv6b_biases))
  out6c = tf.nn.conv2d(relu,
                      conv6c_weights,
                      strides=[1, 1, 1, 1],
                      padding='SAME')
  relu = tf.nn.relu(tf.nn.bias_add(out6c, conv6c_biases))
  ...
  conv = tf.nn.conv2d(relu, conv6de_weights, strides=[1, 1, 1, 1], padding='SAME')
  relu = tf.nn.relu(tf.nn.bias_add(conv, conv6de_biases))

  relu = tf.concat([relu, out6cr], 3)
