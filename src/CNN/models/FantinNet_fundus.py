...
def model(data, train, imagesize, numchannels, numlabels):
  ...
  return outputmap, conv1_weights, conv1_biases, outputshape, out1,  out2
...
def build_feedfoward_ops(train_data_node, train_labels_node, train, imagesize, numchannel, numlabels, reusetest=True):
  ...
  if train:
    ...
    tf.summary.image('outputmap', get_image_summary_disp(outputmap), collections=['train'])
    tf.summary.image('input', get_image_summary(train_data_node), collections=['train'])
    ...
    tf.summary.image('outputmap_test', get_image_summary_disp(outputmapeval),collections=['test'])
    tf.summary.image('input_test', get_image_summary(train_data_node), collections=['test'])
